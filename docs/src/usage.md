...
cannot be expressed as a percentage. Let us verify, that it is indeed the case:
...
Now, to capture the stacktrace type the last line again or hit the up arrow - but do not press enter - press the Meta-s instead. After a short delay, you should see something like this:
```julia
julia> colorant"hsl(80%, 20%, 15%)"
