...

{
    ...
    "title": "Usage",
    "category": "section",
    "text": "Rebugger works from Julia\'s native REPL prompt. Currently there are two important keybindings:Meta-e maps to \"enter\" or \"step in\"\nMeta-s maps to \"stacktrace\" (for commands that throw an error)Meta reliably maps to Esc, and if using Esc you should hit the two keys in sequence rather than simultaneously. For many users Alt (or Option) may be more convenient, as it can be pressed simultaneously with the key. If you\'re on OSX, you may want to enable \"Use option as the Meta key\" in your Terminal settings to avoid the need to press Esc before each Rebugger command."
},
